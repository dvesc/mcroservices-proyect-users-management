...
public class Auth0 {
  ...
  /**
   * FUNCION IMPORTANTE:-------------------------------------------------------
   * Lo que sucede es que uno de los valores de las propiedades se obtiene ejecutando
  ...
  private TokenHolder execute_authRequest (){
    ...
    try { holder = authRequest.execute();} 
  ...
  //CONSTRUCTOR----------------------------------------------------------------
  public Auth0(){} //Un constructor general para cuando no podamos usar @Autowired
  ...
  //VALIDAR TOKEN------------------------------------------------------------
  /**
   * Para validar el token auth0 me dice que se debe usar una libreria (ver el xml)
   * y ella funciona de esta forma; todo se podria decir que es una configuarion para
  ...
  public boolean valid_token(String token){
    try {
        ...
        idTokenVerifier.verify(
            token, 
            null //"NONCE" Es como un string que nosotros esperamos que tenga el token pero aun no funcionaba)
    ...
    public Void update_all(String auth0_id,String name, String  email, String phone_number){
        ...
        if(name != null) new_data.setName(name);
        if (email != null) new_data.setEmail(email);
        if(phone_number != null) new_data.setPhoneNumber(phone_number);
